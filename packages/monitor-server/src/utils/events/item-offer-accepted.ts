...
export default function listenForItemOfferAccepted(
  marketContract: TypeChain.contracts.nftMarketPlaceSol.NftMarketplace,
  chainId: bigint
) {
  const prisma = new PrismaClient();
  marketContract.on(
    marketContract.getEvent("NftMarketplace__ItemOfferAccepted"),
    logListener(async (offerId, offer) => {
      ...
      if (previousOfferMade === null) {
        throw new PrismaClientKnownRequestError(
          "offer should have existed before accepting",
          {
            code: "P2015",
      ...
      await prisma.nftMarketplace__ItemOfferAccepted.create({
        data: {
          chainId,
