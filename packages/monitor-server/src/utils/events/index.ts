...
import listenForItemOfferCancelled from "./item-offer-cancelled";
import listenForItemOfferAccepted from "./item-offer-accepted";
const CHAIN_RPC_URL = {
...
async function setUpEventListenerOnChain(chainId: number) {
  ...
  if (marketContract) {
    ...
    listenForItemOfferCancelled(marketContract, BigInt(chainId));
    listenForItemOfferAccepted(marketContract, BigInt(chainId));
